<template>
  <div class="ay-header">
    <div class="ay-header-left">
      <a class="ay-header-back" @click.preventDefault v-show="leftOptions.showBack" :transition="transition" @click="onClickBack">{{leftOptions.backText}}</a>
      <div class="left-arrow" @click="onClickBack" v-show="leftOptions.showBack" :transition="transition"></div>
      <slot name="left"></slot>
    </div>
    <h1 class="ay-header-title" @click="$emit('on-click-title')"><span v-show="title" :transition="transition">{{title}}</span><slot></slot></h1>
  ...
  methods: {
    onClickBack () {
      if (this.leftOptions.preventGoBack) {
        ...
        if(typeof api != 'undefined') api.pageBack()
