<template>
  <div class="ay-slider">
    <div class="ay-swiper" :style="{height: xheight}">
      <slot></slot>
      <div class="ay-swiper-item" v-for="item in list" @click="clickListItem(item)">
        <a href="javascript:">
          <div class="ay-img" :style="{backgroundImage: buildBackgroundUrl(item.img)}"></div>
          <p class="ay-swiper-desc" v-if="showDescMask">{{item.title}}</p>
        </a>
      </div>
    </div>
    <div :class="[dotsClass, 'ay-indicator', 'ay-indicator-' + dotsPosition]" v-show="showDots">
      <a href="javascript:" v-for="key in length">
        <i v-if="length > 1" class="ay-icon-dot" :class="{'active': key === current}"></i>
      </a>
    </div>
  </div>
</template>
