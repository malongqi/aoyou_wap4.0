<template>
    <div class="ay-indexlist">
        <ul class="ay-indexlist-content" v-el:content :style="{ 'height': currentHeight + 'px', 'margin-right': navWidth + 'px'}">
            <div class="search-content"  slothtml="haschild">
                <div class="title-block currentcity" v-if="city" navtitle="定位">
                    {{locationtitle}}: <span>{{city}}</span>
                </div>
                <div class="title-block" navtitle="历史">
                    {{historytitle}}:
                </div>
                <ul class="hotcity" v-if="history">
                    <li class="hotlist" v-for="item in history">
                        <a  class="checker"  @click="handleResultClick(item)"  :class="{ 'active': city==item.CityName}">{{item.CityName}}</a>
                    </li>
                </ul>
