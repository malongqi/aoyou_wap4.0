<template>
  <a class="ay_cell" href="javascript:">
    <div class="ay_cell_bd ay_cell_primary">
      <p>{{title}}</p>
      <inline-desc v-if="inlineDesc">{{inlineDesc}}</inline-desc>
    </div>
    <div class="ay_cell_ft with_arrow ay-datetime-value">{{value || placeholder}}</div>
  </a>
</template>
...
export default {
  ...
  ready () {
    ...
    this.$el.setAttribute('id', 'ay-datetime-' + uuid)
    this.render()
  },
  computed: {
    pickerOptions () {
      const _this = this
      const options = {
        trigger: '#ay-datetime-' + this.uuid,
        format: this.format,
        value: this.value,
        output: '.ay-datetime-value',
