  ...
  <div class="page single">
    ...
    <view-box>
      <div class="page-inner">
        <div class="header-box" slot="header"><!--0216 F-->
        ...
        <div class="container">
          ...
          <div>
            <div class="block">
              <cell class="title-name" :title="KeyWrodList.keyWordTypeName"></cell>
            </div>
            <div class="tab-con">
            <ul class="list-items">
              <li v-for="KeyWord in KeyWrodList.keyWords"><a class="item" :href="KeyWord.linkUrl"><span>{{KeyWord.keyWordName}}</span></a></li>
            </ul>
            </div>
          </div>
          <div class="h10"></div>
          <!--精选产品-->
          <section class="ay-section" v-if="channelProductList.products || channelProductList.products.length>0">
            <cell  class="title-name" :title="channelProductList.spaceName"></cell>
            <ul class="product-list">
              <li class="list-item" v-for="itemProduct in channelProductList.products">
          ...
          <div v-for="ProductList in ProductGroup">
            <div class="block">
              <cell class="title-name" :title="ProductList.spaceName"></cell>
            </div>
            <section class="pro-cell-box" >
                <a class="product-cell" :href="ProductCell.productUrl" v-for="ProductCell in ProductList.products">
                  <div class="imgbox">
                    <span class="label" :class="ProductCell.productType | class">{{ProductCell.productType | level}}</span>
                      <img src="http://pic1.aoyou.com/wap4.0/index/bg-s.png?imageView2/1/w/162/h/109/q/90" 
                        v-lazyload:opt.fadein="ProductCell.picUrl+'?imageView2/1/w/162/h/109/q/90'" >
                  </div>
                  <div class="inforbox">
                    <div class="price">
            ...
            callback: function(obj) {
                if (obj.ReturnCode == 0) {
                    ...
                    api.post({
                        ...
                        useCache: useCache,
                        useProxy: useProxy,
                        callback: function(cpobj) {
                            if (cpobj.ReturnCode == 0) {
                                 vm.channelProductList = cpobj.Data
                            }
                        }
                    });
                    //关键字数据
                    ...
                    api.post({
                        ...
                        callback: function(kwobj) {
                            if (kwobj.ReturnCode == 0) {
                                 vm.KeyWrodList = kwobj.Data.keyWords[0]
                            }
                        }
                    });
                    //导航关键字数据
                    ...
                    var zdyProductParam={
                      ...
                      type:1,
                      channelGUID:channelGUID,
                      space:'3BA5086E-C093-4F8D-B41D-ED6750999BB2'
                    }
                    api.post({
                        path: api.config.microServiceHost.elementurl,
                        controller: 'Element',
                        action: 'GetProductBySpaceGuid',
                        param: JSON.stringify(zdyProductParam),
                        useCache: useCache,
                        useProxy: useProxy,
                        callback: function(zdycpobj) {
                            if (zdycpobj.ReturnCode == 0) {
                                 vm.ProductGroup = zdycpobj.Data.listappElementSpace
                            }
                        }
                    });
...
<style lang="less">
  ...
  .container{
    width: 100%;/*0216 F*/
    height: 100%;/*0216 F*/
    box-sizing: border-box;/*0216 F*/
    overflow: scroll;/*0216 F*/
  }
  .single{
    .ay_tab_bd{
      padding-top: 44px;
