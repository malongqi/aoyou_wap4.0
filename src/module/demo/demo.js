...
router.on('/module/demo.html/component/center', {
...
router.beforeEach(function (transition) {
  if (transition.to.fullPath !== '/module/demo.html/demo') {
  ...
  if (/\/http/.test(transition.to.path)) {
    ...
    if (/\/demo\/component\/\w+/.test(transition.to.path)) {
      router.go({
        replace: true,
        path: transition.to.path.replace('/module/demo.html/demo', ''),
        append: false
      })
    } else {
      transition.next()
...
router.afterEach(function (transition) {
  if (transition.to.path !== '/module/demo.html/demo') {
    window.scrollTo(0, 0)
  } else {
    window.removeEventListener('scroll', saveDemoScrollTop, false)
