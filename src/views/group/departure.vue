  ...
  <div class="page">
    <headerbar class="headbar">
      ...
      <div class="departciry">
        <span class="citys" @click="">联运政策</span>
      </div>
    </headerbar>
    <view-box class="page-inner-departure">
      ...
      <div class="listprice">
        ...
        <div class="price" :class="{ oneline: item.TransportPolicyCityStr.length<7 }"
             v-for="item in mainData.TransportPolicy.TransportPolicyPriceList">
          <p>出发地：{{item.TransportPolicyCityStr}}</p>
          <span v-if="item.Price!=0">+￥{{item.Price}}<b>起</b></span>
          <span v-if="item.Price==0">免费</span>
        </div>
      </div>
      <div class="flight">出发地 - 中转城市航空公司：{{mainData.TransportPolicy.AirLineName}}</div>
      <div class="tip clearfix"><i class="icon-tip"></i>
        <p>由于机票库存实时更新，国内各出发地往返的最终航班时刻须以实际出票为准，敬请理解！</p></div>
    </view-box>
  </div>
</template>
<style lang="less">
    ...
    methods: {
      ...
      takeData: function () {
        ...
        if(g_selected_data.pid!=0){
          this.mainData = g_mainData
          this.mainData.TransportPolicy = g_departure
        }else{
          window.history.back()
        }
