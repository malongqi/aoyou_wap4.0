window.api = (function () {
    function API() {
        this.config = {
            ...
            microServiceHost: {
                ...
                preferentialmicro: 'http://preferentialmicro.aoyou.com/wap/',
                prefreurl: 'http://preferentialmicro.aoyou.com/wap/',
                qiangurl:'http://qiangmicro.aoyou.com/wap/'
    ...
    function Wap() {
            ...
            Wap.prototype.ajax({
                ...
                success: function (data) {
                    if (ayWeChatShare != undefined) {
                        ...
                        var isWeshop = api.getQueryString("ShopId");
                        ...
                        ayWeChatShare.share({
                            ...
                            success: function (res) {
                                if (isWeshop != null && isWeshop != "") {
                                    $.ajax({
                                        url: 'http://weshop.aoyou.com/Log/AddLog',
                                        data: { 'LogType': 0, 'DeptCode': '0101020101', 'ReferUrl': '跟团', 'Url': "'" + link + "'", 'ShopId': "'" + isWeshop + "'" },
                                        dataType: 'json',
                                        type: 'POST',
        ...
        this.toLogin = function (fromUrl, toUrl, method) {
            location.replace("http://mpass.aoyou.com?from=" + fromUrl + "&forward=" + toUrl);
        }
    ...
    function Android() {
        ...
        this.toLogin = function (fromUrl, toUrl, method) {
            if (window.action.toLogin != null && window.action.toLogin != undefined) {
                window.action.toLogin(JSON.stringify({ "from": fromUrl, "to": toUrl, "method": method }));
            }
            else {
                location.replace("http://mpass.aoyou.com?from=" + fromUrl + "&forward=" + toUrl);
            }
        }
        this.logincallback = function (memberid, method) {
            if (memberid != "-1") {
                this.setCookie("memberid", memberid)
                this.setCookie("isLogin", true)
                location.reload(location.href.replace("memberid", "m"))
            }
            else if (memberid == "-1" && method == "back") {
                this.pageBack()
            }
        }
        this.checkIsLogin=function (flag) {
            if(!flag)
            {
                this.setCookie("isLogin", flag)
            }
        }
        this.telClick = function (tel) {
        ...
        this.getDataFromApp=function(key,param, func){
          var timeInt = parseInt(new Date().getTime())
            Android.prototype.timeInt=timeInt
            Android.prototype.callbackArr.push({ key: timeInt, callback: func });
          window.action.exec(JSON.stringify({"key":key,"what":timeInt,"param":param}))
        }
    }
    ...
    function Ios() {
        ...
        this.toLogin = function (fromUrl, toUrl, method) {
            if (window.webkit.messageHandlers.toLogin != null && window.webkit.messageHandlers.toLogin != undefined) {
                window.webkit.messageHandlers.toLogin.postMessage({ "from": fromUrl, "to": toUrl, "method": method });
            }
            else {
                location.replace("http://mpass.aoyou.com?from=" + fromUrl + "&forward=" + toUrl);
            }
        ...
        this.getDataFromApp=function(key,param, func){
          var timeInt = parseInt(new Date().getTime())
            Ios.prototype.timeInt=timeInt
            Ios.prototype.callbackArr.push({ key: timeInt, callback: func });
            window.webkit.messageHandlers.exec.postMessage(JSON.stringify({"key":key,"what":timeInt,"param":param}))
        }        
    }
    var api = new API();
    var devicetype = 'wap'
    ...
    if (api.getQueryString('memberid') || api.getQueryStringfilter('memberid')) {
        ...
        if (memberid != "" && memberid != -1) {
            ...
            var isLogin = api.getCookie("isLogin")
            if (memberid == -1 && (isLogin == "false" || isLogin == undefined)) {
                if (window.webkit != null && window.webkit != undefined) {
